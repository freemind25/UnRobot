...
export const useMLDetector = (): UseMLDetectorReturn => {
  ...
  const analyzeWithML = useCallback(async (text: string): Promise<HybridAnalysis> => {
    ...
    const heuristicResult: AIAnalysisResult = text.length >= MIN_ANALYSIS_LENGTH ? analyzeText(text) : { score: 0, perplexityScore: 0, burstinessScore: 0, transitionScore: 0, perfectionScore: 0, voiceScore: 0, vocabularyScore: 0, depthScore: 0, structureScore: 0, semanticRepetitionScore: 0, personalizationScore: 0, paraphraseScore: 0, styleScore: 0, paragraphBalanceScore: 0, entropyScore: 0, compressionRatioScore: 0, zipfScore: 0, passiveVoiceScore: 0, humanizationScore: 100, sucksScore: 0, patternCount: 0, checklist: [], details: [], styleFingerprint: emptyFingerprint, classification: "human", classificationLabel: "Texte trop court", humanizationDetection: null };
